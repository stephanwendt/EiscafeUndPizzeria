class ContactMessagesController < ApplicationController
	...
	def create
		@contact_message = ContactMessage.new(contact_message_params, customer: current_customer)

		respond_to do |format|
			if @contact_message.save
				ContactMailer.confirmation_mail_to_customer(current_customer, current_order).deliver_later
				ContactMailer.contact_mail_to_responsible(current_customer, current_order).deliver_later
	...
	def update
		respond_to do |format|
			if @contact_message.update(contact_message_params, customer: current_customer)
